...
void echo(int clientfd)
{
    ...
    {
        //read msgType
        readBytes = readn(clientfd, &buf.msgType, sizeof(buf.msgType));
        if (readBytes == -1)
        ...
        //然后读取数据部分
        readBytes = readn(clientfd, (&(buf.msgData.oltIndex.ifType)), oltSize);
        if (readBytes == -1)
            err_exit("read socket error");
        ...
        //然后将其回写回socket
        if (writen(clientfd, &buf, sizeof(buf.msgLen)+sizeof(buf.msgType)+lenHost+oltSize) == -1)
